...
| 设置与 UI 偏好 | 用户选择 | 类型化设置、标签页/筛选、有限后台任务状态、热力图标题/范围/隐私预览，以及可选显示币种标签和用户手填的每 USD 显示单位 | 保留到重置、清除或卸载；普通 VS Code 设置可能参与 Settings Sync | 插件不主动传输，也不查询汇率 |
...
插件只保存校验后的标签与用户手填乘数，不查询汇率；换算后的估算值统一标记 `≈`，
供应商原生金额（例如真实 usage credits）继续按其上报币种显示。
