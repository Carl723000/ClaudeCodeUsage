...
- AI 조언과 Usage Optimizer는 준비된 요청, 정확한 미리보기, 명시적 보내기, 취소, 엄격한 응답 파싱 경계를 하나로 공유합니다. 기본 또는 백그라운드 AI 요청은 없습니다.
- 권장 사항을 적용한 뒤 신뢰할 수 있는 유사 작업만 버전이 있는 로컬 전후 비교로 묶습니다. 거친 지표, 품질, 적용 범위, 버전만 저장하고 프롬프트, 본문, 경로, session ID는 저장하지 않습니다. 신뢰할 수 있는 쌍이 없으면 근거 부족을 표시합니다.
- Codex 기록 작업은 진행률, 연속 실패 횟수, 다음 실행 가능 시각, 일시 중지 이유를 영구 저장합니다. 최초 한 번의 제한된 무거운 처리를 허용하되 완료 후 동등한 작업을 반복하지 않으며, 실패나 진행 없음 뒤 일반 새로고침이 계속 깨우지 않습니다.
- 최근 30일 중 데이터가 있는 모든 Codex 날짜를 이미 인덱싱된 시간 버킷으로 펼칠 수 있습니다. 클릭할 때 JSONL을 읽지 않으며 Claude / Codex는 설정 시간대와 `HH:00` 형식을 공유합니다.
- 주간 Codex 쿼터 관측은 기존 인덱스 처리 중에 압축되어, 상한이 있는 계정 중립 로컬 기록으로 보존됩니다. 따라서 불규칙한 재설정 경계를 유지할 수 있습니다. 폴링, 자격 증명 읽기, 두 번째 스캐너는 추가하지 않습니다. 한 번 시드된 뒤 변경 없는 새로 고침은 사용량 본문을 0바이트만 읽으며, 로컬 로그에 나타나지 않은 재설정은 추론하지 않습니다.
- 최근 7/30일 보기는 오래되었거나 부풀려진 기간 투영을 거부하고, 시간대 투영이 재구축될 때까지 검증된 일별 집계로 임시 폴백합니다.

패키지 버전은 그대로입니다. 이는 로컬 검증 후보이며 게시된 릴리스가 아닙니다.
