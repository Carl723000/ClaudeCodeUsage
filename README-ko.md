...
- 로컬 Codex의 `sessions/**/*.jsonl`과 `archived_sessions/**/*.jsonl`만 읽습니다. 자격 증명과 DB 파일은 제외되며, 결정론적 인사이트는 프롬프트, 응답, 명령 또는 도구 인수 본문을 검사·사용·보관하지 않습니다.
- **처리된** 값 = 입력 + 출력, **새 입력을 포함한 fresh** = 캐시되지 않은 입력 + 출력, **캐시 입력**은 입력의 부분집합, reasoning은 출력의 부분집합입니다. Codex 비용은 추정하지 않습니다.
- Claude / Codex / Compare 보기에는 모델, effort, 루트 / 자식 작업 비율, approval-reviewer 활동, 인덱스 커버리지, 품질 플래그와 **마지막 관측** 주요 한도 스냅샷이 포함됩니다.
- 로컬 권장 사항은 high effort, 다중 에이전트, 반복 검사, 낮은 캐시 패턴을 알려주고 바로 붙여 넣을 제약을 제공합니다. 백그라운드 인덱스 감시 지연은 기본 30초이며 끄거나 더 길게 설정할 수 있습니다.
