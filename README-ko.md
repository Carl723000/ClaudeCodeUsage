...
![Claude 오늘, 중국어 간체, 어두운 테마](images/v2.3.1/claude-today-zh-CN-dark.png)

![Codex 개요, 중국어 간체, 어두운 테마](images/v2.3.1/codex-overview-zh-CN-dark.png)

![Codex 주간 한도 추정, 영어, 어두운 테마](images/v2.3.1/codex-weekly-estimate-en-dark.png)

![Claude와 Codex 통합 히트맵, 영어, 밝은 테마](images/v2.3.1/compare-heatmap-en-light.png)

위 4장은 실제 대시보드 렌더러, 합성 데이터, VS Code Light+/Dark+ 테마 변수로 생성했습니다. 개인 사용량이나 청구 증거가 아니며, 네이티브 VSIX 설치는 별도로 검증합니다.
...
- 설정은 하나의 통화 드롭다운으로 정리되며 기본값은 USD입니다. 다른 13개 표시 통화는
  2026-09-09 ECB 참조 환율의 고정 스냅샷을 사용합니다. 환율은 편집하거나 온라인으로
  가져오지 않으며 기반 추정치는 계속 USD로 유지됩니다.
- Claude와 Codex의 전체 기간에서 월 → 일, 최근 30일에서 이미 집계된 일 → 시간
  드릴다운을 지원합니다. 마우스, 키보드, 포커스, 중첩 접기, 다시 로드한 뒤 복원 동작이
  일치하며 클릭할 때 JSONL을 읽지 않습니다.
- 일반 실시간 새로고침은 현재 탭, 확장 경로, 선택한 지표/시간, Optimizer 초안,
  키보드 포커스와 가까운 스크롤 위치를 유지합니다.
- 모든 차트는 provider가 포함된 현지화 접근성 이름을 제공합니다. 빈 시간도 축, 표,
  툴팁과 선택 가능한 0 값은 유지하지만 각 막대 위의 0 표시는 생략합니다. 실제 사용량의
  가격을 알 수 없을 때는 계속 `—`를 표시합니다.
- provider watcher는 제한된 백오프로 복구하며, 변경되지 않은 Codex 스레드 제목 인덱스는
  검증된 메모리 캐시를 재사용해 백그라운드 작업을 줄입니다.

검토된 Release 초안은 `v2.3.2`이며, 수동 게시 워크플로가 해당 tag에서 패키지 버전을 설정합니다.

## v2.3.1 새로운 기능

- 정확한 가격 및 컨텍스트 메타데이터가 **GPT-6 Astra**(`gpt-6-astra`,
  105만 토큰)와 **Claude Fable 5.1 / Mythos 5.1**
  (`claude-fable-5-1` / `claude-mythos-5-1`, 100만 토큰)을 지원합니다.
  Fable 5.1은 전용 캐시 읽기 요금인 `$0.25 / 백만 토큰`을 사용하며 기존
  Fable 5 기록은 바꾸지 않습니다. GPT-6 API 등가액은 공식 Standard 단기
  컨텍스트 요금을 사용합니다. 로컬 집계 로그로는 개별 요청이 272K 입력을
  넘었는지 확인할 수 없으므로 해당 요청 전체에 적용되는 추가 요금은 포함하지
  않습니다.
- Claude 추정치는 Anthropic 직접 요금 또는 AWS Bedrock in-region 온디맨드 요금을
  선택해 현재 Opus·Sonnet·Haiku 계열에 적용할 수 있습니다. 백엔드를 바꾸면 변경되지
  않은 로그도 즉시 다시 계산하며, Sonnet 5에는 2026년 8월 31일 출시 프로모션 종료
  이후의 표준 요금을 사용합니다.
- 최근 30일은 설정 시간대의 오늘과 이전 29일입니다. 합계가 대조되고 반복 새로고침이 안정적이며 월은 오래된 순서로 표시됩니다. Claude Workflows 요약도 같은 롤링 범위를 사용하고 설정 시간대의 시작일에 따라 각 실행을 집계합니다. 차트 지표를 전환하거나 드릴다운 레이블을 표시할 때도 일/월 사용량 키가 호스트 시간대 때문에 전날이나 전월로 밀리지 않습니다. AI 조언과 Usage Optimizer의 일시 중지 만료일도 설정 시간대와 UI 언어로 표시합니다.
- 구조화 reasoning effort를 모델 이름에서 추측하지 않습니다. 0이 아닌 `unknown`은 이유를 설명하고 0은 숨깁니다.
- 제한된 익명 쿼터 관측이 불규칙·당일 재설정을 보존합니다. 같은 창의 유효 비율로 현재 기간을 포함한 총한도와 미사용 내구성 추정치를 표시합니다. 근사·미귀속 증거는 낮은 신뢰도로 표시합니다. 로컬 Codex 쿼터 계열이 겹쳐도 현재 기간은 마지막 실제 관측으로 신뢰도 낮은 혼합 추정을 표시하고, 귀속이 모호한 완료 기간만 사용분 전용으로 유지합니다.
- 비교 탭은 기존 일별 집계로 Claude + Codex 통합 활동 히트맵을 표시하며 단일 공급자에서도 작동합니다.
- 미리보기 중심 공유 스튜디오는 결정적 로컬 SVG, 공유 카드, Markdown, 제목/기간, 개인정보 미리보기와 조절 가능한 색상을 지원합니다. Academic Violet이 기본이며 강도 매핑은 분위수(기본), 로그, 선형 중에서 선택할 수 있습니다. 공개 GitHub 전송은 정확한 대상을 별도로 확인합니다.
- Codex Token 구성은 겹치지 않는 「캐시되지 않은 입력 / 캐시 입력 / 출력」스택 위에 **캐시되지 않은 사용량**(캐시되지 않은 입력 + 출력)을 명확히 표시합니다. 추론은 계속 출력의 부분집합입니다.
- Codex는 인덱싱 중에도 부드럽게 스크롤됩니다. 스크롤 상태는 동작이 끝난 뒤 한 번만 저장되고 실시간 인덱스 진행률은 페이지 전체를 다시 만들지 않고 텍스트만 갱신합니다. 모델별 녹색 요약은 Claude와 같은 의미로 정확히 일치하는 모델의 API 등가 비용과 가격 적용 범위를 표시합니다. 알 수 없는 모델은 `—`, effort는 중립색의 캐시되지 않은 Token 값으로 유지합니다.
- OpenAI 호환 Optimizer 요청은 `reasoning_effort`만 보내고 지원되지 않는 최상위 `thinking`을 보내지 않아 [#94](https://github.com/ClaudeCodeUsage/ClaudeCodeUsage/issues/94)를 해결합니다.
- Claude/Codex는 밀도, 계층, 포커스, ARIA, 좁은 폭, 밝고 어두운 테마 token을 공유하되 지표 의미는 분리합니다.
