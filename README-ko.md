...
모두 동일한 설치된 v2.3.1 VSIX와 합성 데이터에서 캡처했으며 계정, 경로, 프로젝트, 스레드 또는 로그 본문을 포함하지 않습니다.
...
- 최근 30일은 설정 시간대의 오늘과 이전 29일입니다. 합계가 대조되고 반복 새로고침이 안정적이며 월은 오래된 순서로 표시됩니다.
- 구조화 reasoning effort를 모델 이름에서 추측하지 않습니다. 0이 아닌 `unknown`은 이유를 설명하고 0은 숨깁니다.
- 제한된 익명 쿼터 관측이 불규칙·당일 재설정을 보존합니다. 같은 창의 유효 비율로 주간 총한도를 추정하며 귀속이 모호하면 사용분만 표시합니다.
- 비교 탭은 기존 일별 집계로 Claude + Codex 통합 활동 히트맵을 표시하며 단일 공급자에서도 작동합니다.
- 결정적 로컬 SVG, 공유 카드, Markdown, 제목/기간, 개인정보 미리보기를 지원합니다. 공개 GitHub 전송은 정확한 대상을 별도로 확인합니다.
...
- Claude와 Codex의 전체 기간 / 비교 보기는 로컬 Token 로그에서 과거 사용 등가치를 직접 계산합니다. 가장 최근의 유효한 공식 재설정 관측을 기준으로 고유하고 겹치지 않는 주간 기간열을 만들며 각 사용 이벤트는 한 기간에만 집계됩니다. 사용할 수 있는 관측이 없으면 사용분 전용 기록만 UTC 월요일부터 월요일까지의 달력 주로 되돌아갑니다. 겹치면서 정렬되지 않은 미래 재설정은 실제로 다른 quota series인 경우에만 충돌로 처리합니다. 같은 series의 관측은 하나의 series로 유지하며 낮은 신뢰도의 근사치로 표시할 수 있습니다. 두 번째 「현재 기간」을 만들지 않습니다. 기간 범위와 재설정 시각은 따로 표시합니다. Codex의 account-wide `codex` 관측은 과거와 현재 기간 모두를 장식할 수 있으며, 재설정 시각이 7일 격자에서 벗어나면 관측 시각이 포함된 표시 기간에 매핑합니다. 파일 출처, 재설정 이동 또는 일별 슬라이스의 경계 통과가 있으면 추정은 가능하지만 신뢰도를 낮추고 근사치로 표시합니다. 파일 키는 계정 ID가 아니므로 같은 home의 대상 파일을 함께 사용합니다. 다른 quota series이거나 관측이 없는 기간은 사용된 등가치만 표시하고 계정 분리를 만들어 내지 않습니다. 현재 미사용 값은 표시하지 않으며 과거 미사용 값은 총한도 추정이 있을 때만 표시합니다. Codex 사용량은 일별 슬라이스로 저장되고 Token은 한 기간에만 집계됩니다. 이 표시 규칙은 인덱스 schema를 변경하거나 재구축을 유발하지 않습니다. 현재 공식 API 단가를 전체 기록에 일관되게 적용합니다. 이는 청구액이나 공식 구독 가격이 아닌 대리 추정치입니다. 이 패널은 기본으로 켜지며 설정의 `showWeeklyEquivalentValue`에서 끌 수 있습니다. Claude는 이 릴리스부터 프로필별 한도 관측을 기록합니다.
...
구조화 근거에 없는 재설정은 복원할 수 없습니다. 여러 로그인 귀속이 모호하면 사용분만 표시합니다.
