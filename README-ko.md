...
- **AI 조언**(선택) — 먼저 로컬 근거와 설명 가능한 행동을 보여 줍니다. 선택형 BYOK 개인화는 기본적으로 집계만 포함하고 프롬프트 샘플에는 별도 동의가 필요합니다. 전체 요청을 정확히 미리 본 뒤 별도의 보내기 동작을 수행하며, 유용함 / 유용하지 않음 / 적용됨 피드백은 로컬에만 남습니다.
