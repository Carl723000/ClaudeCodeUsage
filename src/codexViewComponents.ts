...
export function getCodexDocumentIdentity(
  provider: 'codex' | 'compare',
  copy: CodexViewCopy,
  locale: string,
): { lang: string; title: string } {
  return {
    lang: locale,
    title: provider === 'compare' ? copy.compareTitle : copy.title,
  };
}
...
export interface ProviderCompareRenderOptions {
...
function defaultDuration(value: number): string {
  return formatLocalizedDuration(value, 'en');
}
...
function scopePanel(
  ...
  copy: CodexViewCopy,
  format: NumberFormatter,
  formatDuration: (milliseconds: number) => string = defaultDuration,
): string {
...
export function dailyTable(
  ...
  const body = rows
    .map(
      (row) => `<tr><td class="date-cell">${escapeHtml(row.day)}</td><td class="number-cell">${formatted(format, row.total.processed)}</td><td class="number-cell">${formatted(format, row.total.fresh)}</td><td class="number-cell">${formatted(format, row.total.input)}</td><td class="number-cell">${formatted(format, row.total.cachedInput)}</td><td class="number-cell">${formatted(format, row.total.output)}</td><td class="number-cell">${formatted(format, row.total.reasoning)}</td><td class="number-cell">${formatted(format, row.threads)}</td><td class="number-cell">${formatted(format, row.childThreads)}</td><td class="number-cell">${formatted(format, row.approvalReviewerThreads)}</td></tr>`,
    )
    .join('');
  return `<section class="daily-breakdown"><h3>${escapeHtml(copy.daily)}</h3><div class="daily-table-container codex-scroll-region"><table class="daily-table"><thead><tr><th>${escapeHtml(copy.date)}</th><th class="number-cell">${escapeHtml(copy.processed)}</th><th class="number-cell">${escapeHtml(copy.fresh)}</th><th class="number-cell">${escapeHtml(copy.input)}</th><th class="number-cell">${escapeHtml(copy.cachedInput)}</th><th class="number-cell">${escapeHtml(copy.output)}</th><th class="number-cell">${escapeHtml(copy.reasoning)}</th><th class="number-cell">${escapeHtml(copy.threads)}</th><th class="number-cell">${escapeHtml(copy.childThreads)}</th><th class="number-cell">${escapeHtml(copy.approvalReviewers)}</th></tr></thead><tbody>${body}</tbody></table></div></section>`;
}
...
function monthlyTable(
  ...
  if (rows.length === 0) {
    return `<section class="daily-breakdown"><h3>${escapeHtml(copy.monthly)}</h3><p>${escapeHtml(copy.noMonthlyData)}</p></section>`;
  }
  const body = rows
    .map(
      (row) => `<tr><td class="date-cell">${escapeHtml(row.period)}</td><td class="number-cell">${formatted(format, row.total.processed)}</td><td class="number-cell">${formatted(format, row.total.fresh)}</td><td class="number-cell">${formatted(format, row.total.input)}</td><td class="number-cell">${formatted(format, row.total.cachedInput)}</td><td class="number-cell">${formatted(format, row.total.output)}</td><td class="number-cell">${formatted(format, row.total.reasoning)}</td><td class="number-cell">${formatted(format, row.threads)}</td></tr>`,
    )
    .join('');
  return `<section class="daily-breakdown"><h3>${escapeHtml(copy.monthly)}</h3><div class="daily-table-container codex-scroll-region"><table class="daily-table"><thead><tr><th>${escapeHtml(copy.date)}</th><th class="number-cell">${escapeHtml(copy.processed)}</th><th class="number-cell">${escapeHtml(copy.fresh)}</th><th class="number-cell">${escapeHtml(copy.input)}</th><th class="number-cell">${escapeHtml(copy.cachedInput)}</th><th class="number-cell">${escapeHtml(copy.output)}</th><th class="number-cell">${escapeHtml(copy.reasoning)}</th><th class="number-cell">${escapeHtml(copy.threads)}</th></tr></thead><tbody>${body}</tbody></table></div></section>`;
}
...
function periodChart(
  ...
  const maxProcessed = Math.max(0, ...ordered.map((row) => row.total.processed));
  ...
  const buttons = metrics
    .map(
      ([key, label]) => `<button class="chart-tab ${key === 'processed' ? 'active' : ''}" data-codex-chart-button="${escapeHtml(chartId)}:${key}" data-codex-action="select-chart-metric" data-codex-chart-id="${escapeHtml(chartId)}" data-codex-chart-metric="${key}" aria-pressed="${key === 'processed' ? 'true' : 'false'}">${escapeHtml(label)}</button>`,
    )
    .join('');
  const bars = ordered
    .map((row) => {
      ...
      const labelAttributes = metrics
        ...
        .join(' ');
      const height = maxProcessed > 0
        ? Math.max(2, Math.round((row.total.processed / maxProcessed) * chartHeight))
        : 2;
      const chartLabel = `${escapeHtml(row.label)} · ${escapeHtml(copy.processed)}: ${formatted(format, row.total.processed)}`;
      return `<div class="hc-col"><div class="hc-barval codex-chart-value" data-codex-chart-value>${formatted(format, row.total.processed)}</div><button type="button" class="chart-bar cache-creation-bar codex-chart-bar" style="height:${height}px" data-codex-chart="${escapeHtml(chartId)}" data-row-label="${escapeHtml(row.label)}" data-processed="${Math.max(0, row.total.processed)}" data-fresh="${Math.max(0, row.total.fresh)}" data-output="${Math.max(0, row.total.output)}" data-reasoning="${Math.max(0, row.total.reasoning)}" data-sessions="${Math.max(0, row.threads)}" data-threads="${Math.max(0, row.threads)}" ${labelAttributes} data-label-threads="${formatted(format, row.threads)}" aria-label="${chartLabel}" title="${chartLabel}"></button></div>`;
    })
    .join('');
...
function limitWindowLabel(limit: CodexLimitView, ctx: CodexRenderContext): string {
  if (limit.windowMinutes === 300) {
    return ctx.copy.fiveHourWindow;
  }
  if (limit.windowMinutes === 10_080) {
    return ctx.copy.weeklyWindow;
  }
...
function threadTable(
  ...
  const projectRoles = new Map<string, CodexThreadUsageView['role']>();
  for (const row of rows) {
    if (!projects.has(row.projectViewKey) ||
        (row.role === 'root' && projectRoles.get(row.projectViewKey) !== 'root')) {
      projects.set(row.projectViewKey, row.projectName ?? copy.unidentifiedProject);
      projectRoles.set(row.projectViewKey, row.role);
    }
  }
  ...
  const periodFilter = requestedPeriod &&
    ...
    ? requestedPeriod
    : '';
  const dateFilter = typeof filters.date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(filters.date)
    ? filters.date
    : '';
  const hasFilters = Boolean(
    normalizedQuery || roleFilter || projectFilter || modelFilter ||
    effortFilter || periodFilter || dateFilter,
  );
  const matching = rows.filter((row) => {
    ...
    return (!normalizedQuery || searchable.includes(normalizedQuery)) &&
      ...
      (!dateFilter || row.dayMembership.includes(dateFilter));
  });
  ...
  const chips = [
    ...
    chip('date', copy.date, dateFilter),
  ].join('');
  ...
  const filterToolbar = `<div class="sess-filters codex-thread-filters">
    ...
    ${periodNote}<div class="codex-filter-chips" aria-label="${escapeHtml(copy.activeFilters)}" data-codex-filter-chips data-codex-search-label="${escapeHtml(copy.searchThreads)}" data-codex-date-label="${escapeHtml(copy.date)}">${chips}</div><button class="btn-secondary" data-codex-action="clear-filters"${hasFilters ? '' : ' hidden'}>${escapeHtml(copy.clearFilters)}</button>
  </div>`;
  const body = displayed
    .map((row) => {
      ...
      const childToggle = childCount > 0
        ? `<button class="group-toggle codex-disclosure" aria-expanded="true" title="${escapeHtml(copy.expand)}" data-codex-action="toggle-thread-children" data-codex-thread-key="${escapeHtml(row.viewKey)}">▼</button>`
        : '';
  ...
  const th = (key: string, label: string, legacyKey = key): string => {
    const numeric = ['processed', 'fresh', 'cache', 'output', 'reasoning', 'duration'].includes(key);
    return `<th class="sortable${numeric ? ' number-cell' : ''}${key === 'title' ? '' : ' codex-wide-only'}" data-sortkey="${legacyKey}" data-codex-sort-key="${key}" aria-sort="none"><button type="button" class="codex-sort-button" aria-label="${escapeHtml(`${copy.sortBy}: ${label}`)}" data-codex-action="sort-sessions" data-codex-sort-key="${key}">${escapeHtml(label)}</button></th>`;
  };
  return `<section class="daily-breakdown" data-codex-session-layout="${hasFilters ? 'flat' : 'tree'}"><h3>${escapeHtml(copy.sessions)}</h3><p class="model-details"><span data-codex-thread-visible data-codex-result-count aria-live="polite" aria-atomic="true">${formatted(format, displayed.length)}</span>/${formatted(format, totalCount)} ${escapeHtml(copy.sessions)}</p>${filterToolbar}<div class="daily-table-container codex-scroll-region"><table class="daily-table sortable-table codex-session-table" data-codex-sort-table="sessions"><thead><tr>${th('title', copy.threadLabel)}${th('recent', copy.date, 'time')}${th('role', copy.role)}${th('project', copy.projectLabel)}${th('model', copy.models)}${th('effort', copy.efforts)}${th('processed', copy.processed)}${th('fresh', copy.fresh)}${th('cache', copy.cacheShare)}${th('output', copy.output)}${th('reasoning', copy.reasoning)}${th('duration', copy.duration)}</tr></thead><tbody>${body}</tbody></table></div></section>`;
}
...
function projectTable(
  ...
  const body = view.projects
    .map((project) => {
      ...
      return `<tr class="sort-row codex-project-row" data-codex-project-view-key="${escapeHtml(project.viewKey)}" data-sort-name="${escapeHtml(name.toLowerCase())}" data-sort-lastactive="${Math.max(0, project.lastActiveAt)}" data-sort-processed="${Math.max(0, project.scope.total.processed)}" data-sort-fresh="${Math.max(0, project.scope.total.fresh)}" data-sort-output="${Math.max(0, project.scope.total.output)}" data-sort-reasoning="${Math.max(0, project.scope.total.reasoning)}" data-sort-roots="${Math.max(0, project.scope.rootTasks)}" data-sort-children="${Math.max(0, project.scope.childThreads)}"><td><button class="group-toggle codex-disclosure" aria-label="${escapeHtml(copy.expand)} ${escapeHtml(name)}" aria-expanded="false" data-codex-action="project-sessions" data-codex-project-view-key="${escapeHtml(project.viewKey)}">▶</button><strong>${escapeHtml(name)}</strong>${directory}<div class="model-details">${formatted(format, shown)}/${formatted(format, project.threadCount)} ${escapeHtml(copy.sessions)}</div></td><td class="codex-wide-only">${escapeHtml(formatDateTime(project.lastActiveAt))}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.total.processed)}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.total.fresh)}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.total.output)}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.total.reasoning)}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.rootTasks)}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.childThreads)}</td><td class="number-cell codex-wide-only">${formatted(format, project.scope.approvalReviewerThreads)}</td></tr><tr class="sort-child codex-project-detail-row" data-codex-project-detail="${escapeHtml(project.viewKey)}" hidden><td colspan="9">${threads || escapeHtml(copy.noThreadData)}<button type="button" class="btn-secondary codex-project-sessions-action" data-codex-action="view-project-sessions" data-codex-project-view-key="${escapeHtml(project.viewKey)}">${escapeHtml(`${copy.viewAllSessions}: ${name}`)}</button></td></tr>`;
    })
    .join('');
  const th = (key: string, label: string): string => {
    const numeric = ['processed', 'fresh', 'output', 'reasoning', 'roots', 'children'].includes(key);
    return `<th class="sortable${numeric ? ' number-cell' : ''}${key === 'name' ? '' : ' codex-wide-only'}" data-sortkey="${key}" data-codex-sort-key="${key}" aria-sort="none"><button type="button" class="codex-sort-button" aria-label="${escapeHtml(`${copy.sortBy}: ${label}`)}" data-codex-action="sort-projects" data-codex-sort-key="${key}">${escapeHtml(label)}</button></th>`;
  };
  return `<section class="daily-breakdown"><h3>${escapeHtml(copy.projects)}</h3><div class="daily-table-container codex-scroll-region"><table class="daily-table sortable-table" data-codex-sort-table="projects"><thead><tr>${th('name', copy.projectLabel)}${th('lastactive', copy.lastActive)}${th('processed', copy.processed)}${th('fresh', copy.fresh)}${th('output', copy.output)}${th('reasoning', copy.reasoning)}${th('roots', copy.rootTasks)}${th('children', copy.childThreads)}<th class="number-cell codex-wide-only">${escapeHtml(copy.approvalReviewers)}</th></tr></thead><tbody>${body}</tbody></table></div></section>`;
}
...
function behaviorPanel(
  ...
  return `<section class="codex-behavior">
    ...
    ${panels}
    <details class="model-item codex-coverage"><summary>${escapeHtml(copy.coverage)} · ${escapeHtml(copy.quality)}</summary><p><strong>${escapeHtml(copy.indexedLogEntries)}</strong>: ${formatted(format, view.coverage.indexedFiles)}/${formatted(format, view.coverage.totalFiles)} · <strong>${escapeHtml(copy.indexedStorage)}</strong>: ${formatted(formatBytes, view.coverage.indexedBytes)}/${formatted(formatBytes, view.coverage.totalBytes)} · ${escapeHtml(view.coverage.complete ? copy.complete : copy.partial)}<br><strong>${escapeHtml(copy.quality)}</strong>: ${quality}</p></details>
...
export function renderCodexPrimaryNav(
  ...
  const pages = [
    ...
    { key: 'explore', label: copy.explore },
    {
      key: 'recommendations',
...
function renderTrendSection(ctx: CodexRenderContext): string {
  ...
  const panels = scopeRows.map(({ key, scope, rows, daily }, index) => {
    ...
    const initialSummary = rows.map((row) =>
      `${row.label} · ${copy.processed}: ${formatted(format, row.total.processed)}`,
    ).join('; ');
    const bars = rows.map((row) => {
      ...
      const dateAction = daily
        ...
        : '';
      const chartLabel = `${escapeHtml(row.label)} · ${escapeHtml(copy.processed)}: ${formatted(format, row.total.processed)}`;
      return `<div class="hc-col"><span class="hc-barval codex-chart-value" data-codex-chart-value>${formatted(format, row.total.processed)}</span><button type="button" class="chart-bar cache-creation-bar codex-chart-bar" data-codex-chart-bar data-row-label="${escapeHtml(row.label)}" data-processed="${Math.max(0, row.total.processed)}" data-fresh="${Math.max(0, row.total.fresh)}" data-output="${Math.max(0, row.total.output)}" data-reasoning="${Math.max(0, row.total.reasoning)}" data-sessions="${Math.max(0, row.threads)}" ${labels}${dateAction} style="height:${Math.max(2, Math.round((row.total.processed / maxProcessed) * 100))}px" aria-label="${chartLabel}" title="${chartLabel}"></button><span class="hc-xlabel">${escapeHtml(row.label)}</span></div>`;
    }).join('');
    const tableRows = rows.map((row) => {
      ...
      return `<tr${rowAttribute}><td class="date-cell">${escapeHtml(row.label)}</td><td class="number-cell">${formatted(format, row.total.processed)}</td><td class="number-cell">${formatted(format, row.total.fresh)}</td><td class="number-cell">${formatted(format, row.total.output)}</td><td class="number-cell">${formatted(format, row.threads)}</td></tr>`;
    }).join('');
    const table = rows.length
      ? `<div class="daily-table-container codex-scroll-region"><table class="daily-table"><thead><tr><th>${escapeHtml(copy.date)}</th><th class="number-cell">${escapeHtml(copy.processed)}</th><th class="number-cell">${escapeHtml(copy.fresh)}</th><th class="number-cell">${escapeHtml(copy.output)}</th><th class="number-cell">${escapeHtml(copy.threads)}</th></tr></thead><tbody>${tableRows}</tbody></table></div>`
...
function taskPanel(ctx: CodexRenderContext): string {
  ...
  for (const item of view.qualityFlags) {
    const label = Object.prototype.hasOwnProperty.call(copy.qualityFlagLabels, item.flag)
      ? copy.qualityFlagLabels[item.flag as CodexQualityFlag]
      : copy.qualityFlagUnknown;
    qualityCounts.set(label, (qualityCounts.get(label) ?? 0) + Math.max(0, item.count));
  }
  const quality = qualityCounts.size > 0
    ? [...qualityCounts].map(([label, count]) => `${escapeHtml(label)}: ${formatted(format, count)}`).join(', ')
    : copy.complete;
  return `${renderLimitsSection(ctx)}${renderRecentTaskSection(ctx)}${renderTrendSection(ctx)}<details class="model-item codex-coverage"><summary>${escapeHtml(copy.coverage)} · ${escapeHtml(copy.quality)}</summary><p><strong>${escapeHtml(copy.indexedLogEntries)}</strong>: ${formatted(format, view.coverage.indexedFiles)}/${formatted(format, view.coverage.totalFiles)} · <strong>${escapeHtml(copy.indexedStorage)}</strong>: ${formatted(ctx.formatters.bytes, view.coverage.indexedBytes)}/${formatted(ctx.formatters.bytes, view.coverage.totalBytes)} · ${escapeHtml(view.coverage.complete ? copy.complete : copy.partial)}<br><strong>${escapeHtml(copy.quality)}</strong>: ${quality}</p></details>`;
}
...
function coverageMarker(
  ...
  return `<p class="model-details codex-period-coverage" data-codex-coverage-range="${range}" data-codex-coverage-status="${coverage.complete ? 'complete' : 'partial'}" data-codex-migrated-files="${Math.max(0, coverage.migratedFiles)}" data-codex-total-files="${Math.max(0, coverage.totalFiles)}" data-codex-migrated-bytes="${Math.max(0, coverage.migratedBytes)}" data-codex-total-bytes="${Math.max(0, coverage.totalBytes)}">${escapeHtml(ctx.copy.coverage)}: ${escapeHtml(ctx.copy.indexedLogEntries)} ${formatted(ctx.formatters.number, coverage.migratedFiles)}/${formatted(ctx.formatters.number, coverage.totalFiles)} · ${escapeHtml(ctx.copy.indexedStorage)} ${formatted(ctx.formatters.bytes, coverage.migratedBytes)}/${formatted(ctx.formatters.bytes, coverage.totalBytes)} · ${escapeHtml(coverage.complete ? ctx.copy.complete : ctx.copy.partial)}</p>`;
}
...
export function renderCodexExplore(ctx: CodexRenderContext): string {
  ...
  return `<nav class="chart-tabs codex-explore-views" id="codex-explore-tablist" role="tablist" aria-label="${escapeHtml(copy.explore)}">${viewButtons}</nav>
...
export function renderCodexRecommendations(ctx: CodexRenderContext): string {
  ...
  return `<section class="codex-recommendations"><h3>${escapeHtml(ctx.copy.recommendations)}</h3><div class="chart-tabs codex-recommendation-tabs" role="tablist" aria-label="${escapeHtml(ctx.copy.scope)}">${buttons}</div>${partialNote}${panels}</section>`;
}
...
export function renderProviderCompare(
  ...
  copy: CodexViewCopy = CODEX_COPY_EN,
  options: ProviderCompareRenderOptions = {},
): string {
  const formatNumber = options.formatNumber ?? number;
  const formatDateTime = options.formatDateTime ?? createCodexLocalizedFormatters(
    'en',
  ...
  ): string => `<article class="summary-item provider-compare-card"><h3>${escapeHtml(value.label)}</h3><p class="model-details">${escapeHtml(accountingLabel)}</p><dl><dt>${escapeHtml(copy.input)}</dt><dd>${formatted(formatNumber, value.input)}</dd><dt>${escapeHtml(copy.cachedInput)}</dt><dd>${formatted(formatNumber, value.cache)}</dd><dt>${escapeHtml(copy.output)}</dt><dd>${formatted(formatNumber, value.output)}</dd></dl></article>`;
  return `<section class="provider-compare"><p class="model-details provider-compare-meta"><strong>${escapeHtml(copy.indexedAllTime)}</strong> · ${escapeHtml(copy.updatedAt)}: <time datetime="${new Date(input.updatedAt).toISOString()}">${escapeHtml(formatDateTime(input.updatedAt))}</time></p><div class="provider-compare-grid">${provider(input.claude, copy.claudeTokenAccounting)}${provider(input.codex, copy.codexTokenAccounting)}</div></section>`;
}
