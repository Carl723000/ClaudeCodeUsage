...
const SETTINGS_I18N: Partial<Record<SupportedLanguage, Record<string, { label: string; help: string }>>> = {
  'de-DE': {
    ...
    'showScopedWeekly': { label: 'Wöchentliches Limit pro Modell anzeigen', help: 'Ergänzt den Wochenwert um jede modellspezifische Wochengrenze deines Plans, sobald sie genutzt wurde. Der Name kommt von Anthropic; im Tooltip bleibt die Grenze immer sichtbar.' },
  ...
  'zh-TW': {
    ...
    'showScopedWeekly': { label: '顯示每模型每週限制', help: '在每週數字中一併顯示方案針對特定模型的每週上限；名稱由 Anthropic 提供。關閉此項不會將其從提示框中隱藏。' },
  ...
  'zh-CN': {
    ...
    'showScopedWeekly': { label: '显示每模型每周限制', help: '在每周数字中一并显示方案针对特定模型的每周上限；名称来自 Anthropic。关闭此项不会将其从悬浮提示中隐藏。' },
  ...
  'ja': {
    ...
    'showScopedWeekly': { label: 'モデル別の週間制限を表示', help: 'プランがモデル別に計測する週間上限を週間の数値に含めます。名称は Anthropic 由来で、オフにしてもツールチップには表示されます。' },
  ...
  'ko': {
    ...
    'showScopedWeekly': { label: '모델별 주간 제한 표시', help: '요금제가 모델별로 측정하는 주간 한도를 주간 수치에 함께 표시합니다. 이름은 Anthropic에서 제공하며, 꺼도 툴팁에는 표시됩니다.' },
  ...
  'pt-BR': {
    ...
    'tokenDecimalPlaces': { label: 'Casas decimais dos tokens', help: 'Casas decimais para a exibição compacta de tokens (1.2M / 345.6K). As contagens inteiras completas não são alteradas.' },
    ...
    'showScopedWeekly': { label: 'Mostrar limite semanal por modelo', help: 'Acrescenta ao valor semanal os limites específicos de modelo informados pela Anthropic. Desativar esta opção não os oculta da dica de contexto.' },
    'quotaFiveHourOnly': { label: 'Cota: somente a janela de 5 horas', help: 'Mostra apenas a cota de 5 horas na barra de status e oculta os valores semanais; os detalhes de reinício permanecem na dica de contexto.' },
    'showResetInStatusBar': { label: 'Cota: mostrar contagem regressiva de reinício', help: 'Acrescenta uma contagem regressiva compacta à barra de status; a dica de contexto sempre mostra o horário completo de reinício.' },
    ...
    'dashboardAutoRefresh': { label: 'Atualização automática do painel', help: 'Atualiza o painel quando chega novo uso. Desativado permite apenas atualização manual; a barra de status continua sendo atualizada.' },
  ...
  'id': {
    ...
    'showScopedWeekly': { label: 'Tampilkan batas mingguan per model', help: 'Menambahkan batas mingguan khusus model yang dilaporkan Anthropic ke angka mingguan. Mematikannya tidak menyembunyikan batas tersebut dari tooltip.' },
    ...
    'resetCountdownFormat': { label: 'Kuota: format hitung mundur reset', help: 'Berlaku saat hitung mundur reset diaktifkan: desimal, satuan jam/menit, atau waktu/tanggal lokal komputer.' },
