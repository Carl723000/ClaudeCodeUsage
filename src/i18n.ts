...
const WEEKLY_VALUE_COPY: Record<SupportedLanguage, WeeklyValueCopy> = {
  en: {
    ...
    multiAccount: 'Codex local history may span multiple sign-ins. One coherent window can show low-confidence total and unused durability estimates; overlapping quota series still show used equivalent only. No account split or official balance is invented.',
  ...
  'zh-CN': {
    ...
    multiAccount: 'Codex 本地历史可能跨多个登录。单个内部一致的窗口可显示低可信度总额与未用耐用度估算；额度系列确实重叠时仍只显示已用值。不虚构账号拆分或官方余额。',
  ...
  'zh-TW': {
    ...
    multiAccount: 'Codex 本機歷史可能跨多個登入。單一且內部一致的視窗可顯示低可信度總額與未用耐用度估算；額度系列確實重疊時仍只顯示已用值。不虛構帳號拆分或官方餘額。',
  ...
  ja: {
    ...
    multiAccount: 'Codex のローカル履歴には複数のログインが含まれる場合があります。一貫した単一枠では低信頼度の総量と未使用量の耐久性推定を表示できます。クォータ系列が重複する場合は使用済み等価値のみを表示し、アカウント分割や公式残高を推測しません。',
  ...
  ko: {
    ...
    multiAccount: 'Codex 로컬 기록에는 여러 로그인이 포함될 수 있습니다. 일관된 단일 창은 신뢰도 낮은 총량 및 미사용 내구성 추정을 표시할 수 있습니다. 할당량 계열이 실제로 겹치면 사용 등가치만 표시하며 계정 분리나 공식 잔액을 추정하지 않습니다.',
  ...
  'pt-BR': {
    ...
    multiAccount: 'O histórico local do Codex pode abranger vários logins. Uma janela coerente pode mostrar estimativas de baixa confiança do total e do restante; séries de cota sobrepostas mostram apenas o equivalente usado. Nenhuma divisão de conta ou saldo oficial é inventado.',
  ...
  'de-DE': {
    ...
    multiAccount: 'Der lokale Codex-Verlauf kann mehrere Anmeldungen umfassen. Ein stimmiges Fenster kann Gesamtsumme und Restwert mit niedriger Zuverlässigkeit schätzen; überlappende Kontingentserien zeigen nur den genutzten Gegenwert. Weder Kontotrennung noch offizielles Guthaben werden erfunden.',
  ...
  id: {
    ...
    multiAccount: 'Riwayat lokal Codex dapat mencakup beberapa login. Satu jendela yang konsisten dapat menampilkan perkiraan total dan sisa berkeyakinan rendah; seri kuota yang tumpang tindih hanya menampilkan ekuivalen terpakai. Pemisahan akun atau saldo resmi tidak direka.',
