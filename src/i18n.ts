...
const WEEKLY_VALUE_COPY: Record<SupportedLanguage, WeeklyValueCopy> = {
  en: {
    ...
    multiAccount: 'Codex local history may span multiple sign-ins. One coherent unattributed quota window can show a low-confidence total estimate, but not unused value. If epochs overlap or attribution conflicts, only used equivalent is shown; no account split is invented.',
    boundaryApproximation: 'Codex usage is aggregated by day. If an official reset falls within a recorded day, the affected period may show a low-confidence full-allowance approximation; current unused value remains withheld.',
  ...
  'zh-CN': {
    ...
    multiAccount: 'Codex 本地历史可能跨多个登录。单个内部一致的未归属额度窗口可显示低可信度总额估算，但不显示未用值；epoch 重叠或归属冲突时只显示已用等价值，不虚构账号拆分。',
    boundaryApproximation: 'Codex 用量按日汇总。如果官方重置发生在某个已记录日期内，受影响周期仍可显示低可信度的总额度近似值；当前周期仍不显示未用估算。',
  ...
  'zh-TW': {
    ...
    multiAccount: 'Codex 本機歷史可能跨多個登入。單一且內部一致的未歸屬額度視窗可顯示低可信度總額估算，但不顯示未用值；epoch 重疊或歸屬衝突時只顯示已用等價值，不虛構帳號拆分。',
    boundaryApproximation: 'Codex 用量按日彙總。如果官方重設發生在某個已記錄日期內，受影響週期仍可顯示低可信度的總額度近似值；目前週期仍不顯示未用估算。',
  ...
  ja: {
    ...
    multiAccount: 'Codex のローカル履歴には複数のログインが含まれる場合があります。一貫した単一の未帰属クォータ枠では低信頼度の総量推定を表示できますが、未使用量は表示しません。epoch の重複や帰属競合がある場合は使用済み等価値のみを表示し、アカウント分割を推測しません。',
    boundaryApproximation: 'Codex の使用量は日単位で集計されます。公式リセットが記録日の途中にある場合、影響する期間には低信頼度の総上限近似を表示できます。現在の未使用分は表示しません。',
  ...
  ko: {
    ...
    multiAccount: 'Codex 로컬 기록에는 여러 로그인이 포함될 수 있습니다. 일관된 단일 미귀속 할당량 창은 신뢰도 낮은 총량 추정을 표시할 수 있지만 미사용 값은 표시하지 않습니다. epoch가 겹치거나 귀속이 충돌하면 사용 등가치만 표시하며 계정 분리를 추정하지 않습니다.',
    boundaryApproximation: 'Codex 사용량은 일별로 집계됩니다. 공식 재설정이 기록된 하루 중간에 발생하면 영향을 받는 기간에 낮은 신뢰도의 총한도 근사를 표시할 수 있으며, 현재 미사용 값은 표시하지 않습니다.',
  ...
  'pt-BR': {
    ...
    multiAccount: 'O histórico local do Codex pode abranger vários logins. Uma única janela de cota não atribuída e coerente pode mostrar uma estimativa total de baixa confiança, mas não o valor não usado. Se épocas se sobrepõem ou a atribuição entra em conflito, só o equivalente usado é mostrado; nenhuma divisão de conta é inventada.',
    boundaryApproximation: 'O uso do Codex é agregado por dia. Se uma redefinição oficial ocorrer dentro de um dia registrado, o período afetado pode mostrar uma aproximação de baixa confiança do limite total; o não usado atual continua oculto.',
  ...
  'de-DE': {
    ...
    multiAccount: 'Der lokale Codex-Verlauf kann mehrere Anmeldungen umfassen. Ein einzelnes, stimmiges, nicht zugeordnetes Kontingentfenster kann eine Gesamtschätzung mit niedriger Zuverlässigkeit zeigen, jedoch keinen ungenutzten Wert. Bei überlappenden Epochen oder Zuordnungskonflikten erscheint nur der genutzte Gegenwert; eine Kontotrennung wird nicht erfunden.',
    boundaryApproximation: 'Die Codex-Nutzung wird tageweise aggregiert. Fällt ein offizieller Reset in einen erfassten Tag, kann der betroffene Zeitraum eine Schätzung des Gesamtlimits mit niedriger Sicherheit zeigen; der aktuelle ungenutzte Wert bleibt verborgen.',
  ...
  id: {
    ...
    multiAccount: 'Riwayat lokal Codex dapat mencakup beberapa login. Satu jendela kuota tanpa atribusi yang konsisten dapat menampilkan perkiraan total berkeyakinan rendah, tetapi bukan nilai yang belum dipakai. Jika epoch tumpang tindih atau atribusi bertentangan, hanya ekuivalen terpakai yang ditampilkan; pemisahan akun tidak direka.',
    boundaryApproximation: 'Penggunaan Codex diagregasi per hari. Jika reset resmi terjadi di tengah hari yang tercatat, periode terdampak dapat menampilkan perkiraan batas total dengan keyakinan rendah; sisa saat ini tetap disembunyikan.',
