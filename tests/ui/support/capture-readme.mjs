...
// Run after compile and the visual-token guard: node tests/ui/support/capture-readme.mjs
...
try {
  ...
  for (const [view, options] of Object.entries(views)) {
    const context = await browser.newContext({
      viewport: { width: 1280, height: 940 }, deviceScaleFactor: 1,
    ...
    if (view === 'claude') {
      ...
      await page.screenshot({ ...screenshot, path: resolve(output, 'claude-today-zh-CN-dark.png') });
    } else if (view === 'codex') {
      await page.locator('#tab-month').click();
      await expect(page.locator('#month')).toContainText('未缓存用量');
      await page.screenshot({ ...screenshot, path: resolve(output, 'codex-overview-zh-CN-dark.png') });
    } else if (view === 'weekly') {
      ...
      await panel.screenshot({ ...screenshot, path: resolve(output, 'codex-weekly-estimate-en-dark.png') });
    } else {
      ...
      await panel.screenshot({ ...screenshot, path: resolve(output, 'compare-heatmap-en-light.png') });
    }
