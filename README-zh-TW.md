...
以上四張 v2.3.1 影像使用正式儀表板渲染器、合成資料與 VS Code Light+/Dark+ 主題變數產生，不是個人用量或帳單證據；原生 VSIX 安裝另行驗證。
...
- 設定現收斂為一個幣別下拉選單，預設 USD；另有 13 種常用顯示幣別，採用固定的
  2026-09-09 ECB 參考匯率快照。匯率不可手動調整、不會連線取得，底層估算仍為 USD。
- Claude 與 Codex 的全部時間均支援「月 → 日」，最近 30 天均支援已有彙總的
  「日 → 小時」；滑鼠、鍵盤、焦點、巢狀收合與重新載入還原行為一致，點擊不讀取 JSONL。
- 一般即時重新整理會保留目前分頁、展開鏈、所選指標／小時、最佳化器草稿、
  鍵盤焦點與附近捲動位置。
- 每張圖都有包含 provider 的本地化無障礙名稱。空白小時仍保留座標、表格、提示與
  可選取的 0 值，但不再在每根空柱上方重複標示 0；真正無法定價的活動仍顯示 `—`。
- provider watcher 採用有界退避恢復；未變更的 Codex 執行緒標題索引使用經驗證的
  記憶體快取，減少背景工作。

已審閱的 Release 草稿為 `v2.3.2`；人工發佈工作流程會從該 tag 寫入套件版本。

## v2.3.1 新功能

- 精確定價與上下文中繼資料現已涵蓋 **GPT-6 Astra**（`gpt-6-astra`，105 萬
  token 上下文）以及 **Claude Fable 5.1 / Mythos 5.1**
  （`claude-fable-5-1` / `claude-mythos-5-1`，100 萬 token 上下文）。
  Fable 5.1 採用專屬的 `$0.25 / 百萬 token` 快取讀取價，既有 Fable 5
  歷史資料不變。GPT-6 的 API 等效值採用官方 Standard 短上下文費率；由於
  本機彙總日誌無法判定單次請求是否超過 272K 輸入，因此不計入該門檻以上的
  整個請求附加價。
- Claude 估算可選用 Anthropic 直連或 AWS Bedrock in-region 隨選費率，涵蓋目前
  Opus、Sonnet 與 Haiku 系列；切換後端會立即按新費率重算未變更的本機日誌。
  Sonnet 5 使用 2026 年 8 月 31 日上市優惠結束後的標準費率。
- 最近 30 天是設定時區中的今天與前 29 個日期；總計可核對、重複重新整理穩定，月份由舊到新。Claude 的工作流程摘要使用同一滾動範圍，並依設定時區中的開始日期歸入週期。切換圖表指標或呈現下鑽標籤時，日／月用量鍵不會再受宿主時區影響而倒退一天或一個月；AI 建議與用量最佳化器的暫停截止日期也統一使用設定時區與介面語言。
- 結構化 reasoning effort 不依模型名稱猜測；非零 `unknown` 會解釋，零值不顯示。
- 有界匿名額度觀測保留不規則及同日重設；有效同窗比例可估算總額度與未用耐用度（包括目前週期）。近似或未歸屬證據標為低可信度。即使本機 Codex 額度系列重疊，目前週期仍採用最後一次真實觀測提供低可信度混合估算；歸屬不清的已結束週期維持僅已用值。
- 「比較」預設顯示 Claude + Codex 綜合活動熱力圖，沿用日聚合，也支援單一供應商。
- 預覽優先的分享工作台可匯出確定性本機 SVG、分享卡與 Markdown，並自訂標題/範圍、預覽隱私及調整配色。工作台預設開啟，可在設定用一個開關整體隱藏。預設 Academic Violet；色階映射可選分位數（預設）、對數或線性；GitHub 公開發佈另行確認精確目標。
- Codex Token 構成現在會在互不重疊的「未快取輸入 / 快取輸入 / 輸出」堆疊上方明確顯示 **未快取用量**（未快取輸入 + 輸出）；推理仍是輸出的子集。
- Codex 在索引期間也能順暢捲動：捲動狀態改為操作結束後只儲存一次，即時索引進度只更新文字，不再重建整頁。各模型折疊項的綠色摘要現在與 Claude 一致，顯示精確模型的 API 等效價格並附定價涵蓋說明；未知模型顯示 `—`，推理強度仍以中性色顯示未快取 Token。
- OpenAI 相容的最佳化器請求現在只傳送 `reasoning_effort`，不再附帶不受支援的頂層 `thinking` 參數，解決 [#94](https://github.com/ClaudeCodeUsage/ClaudeCodeUsage/issues/94)。
- Claude/Codex 共用密度、層級、焦點、ARIA、窄幅與深淺色設計 token，但不混淆指標語義。
