...
- **已處理** = 輸入 + 輸出；**未快取用量** = 未快取輸入 + 輸出；**快取輸入**仍是輸入的子集，reasoning 仍是輸出的子集。Codex 不顯示帳單成本；首張摘要卡會針對目前範圍顯示明確標示的 API 等效成本估算，「全部時間」則依同一定價口徑顯示每週趨勢。Claude / Codex / Compare 仍維持各供應商口徑獨立。
- Claude 與 Codex 的「全部時間」/「比較」會直接依本機 Token 日誌計算歷史已用等價值。有真實每週重設觀測時依該節奏對齊；沒有時，僅已用歷史依 UTC 週一至週一的自然週分組。只有視窗存在真實用量比例觀測時，才顯示總額度與未用額度估算。歷史統一套用目前官方 API 單價；這是代理估算，不是帳單或官方訂閱價格。Codex 的僅已用歷史會合併所選 home 中的登入，總額度估算仍綁定實際觀測的重設序列，不會虛構帳號拆分。Claude 從此版本開始依本機 profile 記錄額度觀測。
- 切換到 Codex 後仍沿用既有的今天 / 本月 / 全部時間 / 工作階段 / 專案 / 內容 / 設定結構，並依 Codex 語義調整標籤。兩個供應商共用同一組 render function、HTML class、圖表、表格、間距與響應式規則；Codex 建議使用已索引的 30 天結構化證據。
