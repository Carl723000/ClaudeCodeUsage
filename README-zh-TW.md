...
- 「取得 AI 建議」與用量最佳化工具共用同一套已準備請求、精確預覽、明確傳送、取消與嚴格回應解析邊界；沒有預設或背景 AI 請求。
- 套用建議後，只有可靠的同類工作才會形成版本化的本機前後比較；內容只含粗粒度指標、品質、涵蓋率與版本，不保存提示、正文、路徑或 session ID。沒有可靠配對時會明示證據不足。
- Codex 歷史工作會持久保存進度、連續失敗次數、下次可執行時間與暫停原因。首次可進行一次有界的重處理；完成後不重複，失敗或無進展時也不會被一般重新整理反覆喚醒。
- 最近 30 天內每個有資料的 Codex 日期都可直接展開已索引的小時資料；點擊時不讀取 JSONL。Claude 與 Codex 共用設定時區與 `HH:00` 格式。
- 每週 Codex 額度觀測會在既有索引流程中壓縮成有界、帳戶中性的本機歷史，因此不同的官方重設邊界可以保留下來；不增加輪詢、憑證讀取或第二套掃描器。一次性播種後，未變更的重新整理仍讀取 0 位元組用量正文；若某次重設從未出現在本機日誌行中，擴充功能無法推斷它。
- 最近 7 天 / 30 天檢視會拒絕過期或膨脹的期間投影，在設定時區的投影重建完成前暫時回退到已驗證的每日聚合。

套件版本維持不變；這是供本機驗證的候選，不是已發佈版本。
