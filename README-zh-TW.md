...
- 有界匿名額度觀測保留不規則及同日重設；有效同窗比例可估算總額度與未用耐用度（包括目前週期）。近似或未歸屬證據標為低可信度，真正重疊的額度系列才維持僅已用值。
- 「比較」預設顯示 Claude + Codex 綜合活動熱力圖，沿用日聚合，也支援單一供應商。
- 預覽優先的分享工作台可匯出確定性本機 SVG、分享卡與 Markdown，並自訂標題/範圍、預覽隱私及調整配色。預設 Academic Violet，分位數強度避免少數極端日期壓平一般活躍日；GitHub 公開發佈另行確認精確目標。
