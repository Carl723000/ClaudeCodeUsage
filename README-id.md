...
- Tampilan Sepanjang Waktu / Perbandingan Claude dan Codex menghitung ekuivalen terpakai historis langsung dari log Token lokal. Pengamatan reset resmi terbaru yang valid menjadi jangkar bagi rangkaian periode mingguan yang unik dan tidak tumpang tindih, sehingga setiap peristiwa penggunaan hanya dihitung dalam satu periode. Jika tidak ada pengamatan yang dapat dipakai, riwayat khusus pemakaian kembali ke minggu kalender UTC Senin-ke-Senin. Reset masa depan yang tumpang tindih tetapi tidak selaras dianggap konflik hanya jika berasal dari series kuota yang benar-benar berbeda; pengamatan dari series yang sama tetap menjadi satu series dan dapat ditampilkan sebagai perkiraan berkeyakinan rendah. Reset itu tidak dapat membuat periode berjalan kedua. Rentang periode dan waktu reset ditampilkan terpisah. Pengamatan account-wide `codex` Codex dapat menghias periode historis dan berjalan. Jika waktu reset bergeser dari grid tujuh hari, sampel dipetakan ke periode tampilan yang memuat waktu pengamatan. Ketidakpastian sumber file, pergeseran reset, atau irisan harian yang melintasi batas tetap dapat menghasilkan perkiraan, tetapi menurunkan keyakinan dan diberi label pendekatan. Kunci file bukan identitas akun, sehingga file lokal yang memenuhi syarat dalam satu home dipakai bersama. Series kuota yang benar-benar berbeda atau periode tanpa pengamatan yang dapat dipakai tetap hanya menampilkan ekuivalen terpakai tanpa mengarang pemisahan akun. Nilai sisa saat ini disembunyikan; nilai sisa historis hanya ditampilkan bila ada estimasi total. Penggunaan Codex disimpan sebagai irisan harian dan setiap Token tetap dihitung satu kali. Aturan tampilan ini tidak mengubah schema indeks dan tidak memicu pembangunan ulang. Harga API resmi saat ini diterapkan konsisten pada riwayat. Ini proksi, bukan tagihan atau harga langganan resmi. Panel ini aktif secara default dan dapat disembunyikan di Settings dengan `showWeeklyEquivalentValue`. Claude mulai merekam pengamatan batas per profil sejak rilis ini.
