...
以上截图均来自同一份已安装的 v2.3.1 VSIX 和合成数据，不含账号、路径、项目、
线程或日志正文。
...
- 最近 30 天严格定义为配置时区中的今天及此前 29 个自然日；满足「今天 ≤ 最近 30 天 ≤ 全部时间」，重复刷新/重建不累加，所有分组可核对，月份从旧到新显示。
- 兼容当前和旧版结构化 reasoning effort，不根据模型名猜测；非零 `unknown` 会解释原因，零值不会渲染。
- 有界匿名额度观测可保留不规则、连续乃至同日重置。每个同窗有效已用比例都参与周总额度估算；账号归属冲突时只显示已用值，未用估算不会为负。
- 「对比」默认展示 Claude + Codex 综合活动热力图，直接复用日聚合、支持任一单供应商，并在 tooltip 中分别显示两侧数据。
- 分享闭环包含确定性本地 SVG、综合分享卡、可复制 Markdown、自定义标题/范围和隐私预览；可选的公开 GitHub 发布需要另行确认精确目标。
...
- 一个 Codex home 可能包含多个登录；归属不清时只显示已用值，不虚构账号拆分或未用额度。
