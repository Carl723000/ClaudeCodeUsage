...
- 最近 30 天严格定义为配置时区中的今天及此前 29 个自然日；满足「今天 ≤ 最近 30 天 ≤ 全部时间」，重复刷新/重建不累加，所有分组可核对，月份从旧到新显示。Claude 的工作流摘要使用同一滚动范围，并按配置时区中的开始日期归入周期。切换图表指标或渲染下钻标签时，日/月用量键不会再受宿主时区影响而回退一天或一个月。
