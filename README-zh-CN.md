...
以上四张 v2.3.1 截图使用生产仪表板渲染器、合成夹具及 VS Code Light+/Dark+ 主题变量生成，不是个人用量或账单证据；原生 VSIX 安装单独验收。
...
- 设置现收敛为一个币种下拉栏，默认 USD；另有 13 种常用显示币种，采用固定的
  2026-09-09 ECB 参考汇率快照。汇率不允许手填、不会联网获取，底层估算仍为 USD。
- Claude 与 Codex 的全部时间均支持“月 → 日”，最近 30 天均支持已有聚合的
  “日 → 小时”；鼠标、键盘、焦点、嵌套收起和重载恢复行为一致，点击不读取 JSONL。
- 普通实时刷新会保留当前标签、展开链、所选指标/小时、优化器草稿、键盘焦点和
  附近滚动位置。
- 每张图都有包含 provider 的本地化无障碍名称。空小时仍保留坐标、表格、提示与
  可选中的 0 值，但不再在每根空柱上方重复标 0；真实未定价活动仍显示 `—`。
- provider watcher 使用有界退避恢复；未变化的 Codex 线程标题索引使用经验证的
  内存缓存，减少后台工作。

已审阅的 Release 草稿为 `v2.3.2`；人工发布工作流会从该 tag 写入包版本。

## v2.3.1 新功能

- 精确定价与上下文元数据现已覆盖 **GPT-6 Astra**（`gpt-6-astra`，105 万
  token 上下文）以及 **Claude Fable 5.1 / Mythos 5.1**
  （`claude-fable-5-1` / `claude-mythos-5-1`，100 万 token 上下文）。
  Fable 5.1 使用其专属的 `$0.25 / 百万 token` 缓存读取价，既有 Fable 5
  历史数据不变。GPT-6 的 API 等效值采用官方 Standard 短上下文费率；由于
  本地聚合日志无法判定单次请求是否超过 272K 输入，因此不计入该阈值以上的
  整个请求附加价。
- Claude 估算可选用 Anthropic 直连或 AWS Bedrock in-region 按需费率，覆盖当前
  Opus、Sonnet 与 Haiku 系列；切换后端会立即按新费率重算未变化的本地日志。
  Sonnet 5 使用 2026 年 8 月 31 日上线促销结束后的标准费率。
- 最近 30 天严格定义为配置时区中的今天及此前 29 个自然日；满足「今天 ≤ 最近 30 天 ≤ 全部时间」，重复刷新/重建不累加，所有分组可核对，月份从旧到新显示。Claude 的工作流摘要使用同一滚动范围，并按配置时区中的开始日期归入周期。切换图表指标或渲染下钻标签时，日/月用量键不会再受宿主时区影响而回退一天或一个月；AI 建议和用量优化器的暂停截止日期也统一使用配置时区与界面语言。
...
- 预览优先的分享工作台包含确定性本地 SVG、综合分享卡、可复制 Markdown、自定义标题/范围、隐私预览和可调配色。工作台默认开启，可在设置中用一个开关整体隐藏。默认采用 Academic Violet；色阶映射可选分位数（默认）、对数或线性；可选的公开 GitHub 发布需要另行确认精确目标。
- Codex Token 构成现在会在互不重叠的「未缓存输入 / 缓存输入 / 输出」堆叠上方明确显示 **未缓存用量**（未缓存输入 + 输出）；推理仍是输出的子集。
- Codex 在索引期间也能顺畅滚动：滚动状态改为操作结束后只保存一次，实时索引进度只更新文字，不再重建整页。分模型折叠项的绿色摘要现在与 Claude 一致，显示精确模型的 API 等效价格并附定价覆盖说明；未知模型显示 `—`，推理强度仍以中性色显示未缓存 Token。
- OpenAI 兼容的优化器请求现在只发送 `reasoning_effort`，不再附带不受支持的顶层 `thinking` 参数，解决 [#94](https://github.com/ClaudeCodeUsage/ClaudeCodeUsage/issues/94)。
...
- **模型专属每周上限**（`showScopedWeekly`，可选开启）：显示 Anthropic
  ...
  模型专属额度功能演进而来。
- **AI 建议 2.0 的兼容基础**：继续支持自备 key 的 Anthropic（`/v1/messages`）与 OpenAI 兼容端点（`advice.apiFormat`）、超时、重试和 curl 兜底；v2.3.1 将旧入口收敛到统一的本地证据、完整请求预览、明确发送和反馈边界。订阅凭据从不用于模型请求。
