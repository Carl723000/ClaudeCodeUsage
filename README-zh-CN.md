...
- **已处理** = 输入 + 输出；**新鲜** = 未缓存输入 + 输出；**缓存输入**仍是输入的子集，reasoning 仍是输出的子集。Codex 不估算美元成本；Claude / Codex / Compare 会保持各供应商的统计口径独立。
- 三个主页面沿用现有仪表板的视觉语言：**概览**提供最近任务 / 最近 7 天 / 最近 30 天 / 全部时间的摘要、趋势、构成、最后观测额度与最近任务；**探索**包含项目、会话、模型与推理强度，可搜索、筛选、排序、下钻并查看父子谱系；**优化建议**跟随所选范围。
...
- 最近 7 天与 30 天按配置时区中的事件发生日精确切片。迁移或覆盖不完整时会明确显示为**部分**。额度是本地日志中**最后观测**到的快照，不是实时订阅或账单数据。
- 每条建议仅在所选范围有证据时展示观测、易读证据、结构性代理指标解释和条件式行动；没有证据就不会生成泛化建议。
- 持久索引保存使用本机盐值生成的假名化键、数值与结构聚合，以及经过脱敏的项目、目录、agent、模型、effort、角色、时间和质量元数据；绝不保存原始 ID、完整路径或仓库 URL、线程标题或对话正文。
- 设置是会返回上一个主页面的供应商感知辅助页，并非第四个主标签页。Codex 数据收集和本地 Codex 建议可分别关闭；后台监听延迟可配置（默认 30 秒，也可关闭或选更长间隔）。
