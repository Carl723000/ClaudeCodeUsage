...
- AI アドバイスと Usage Optimizer は、準備済みリクエスト、完全一致プレビュー、明示送信、キャンセル、厳格な応答解析を一つの境界で共有します。既定またはバックグラウンドの AI リクエストはありません。
- 推奨を適用した後、信頼できる類似タスクだけをバージョン付きローカル比較にできます。保存するのは粗い指標、品質、カバレッジ、バージョンのみで、プロンプト、本文、パス、session ID は保存しません。信頼できる組がなければ根拠不足と表示します。
- Codex の履歴処理は進捗、連続失敗回数、次回実行可能時刻、停止理由を永続化します。初回は一度だけ有界な重い処理を許可し、完了後に同等処理を繰り返さず、失敗や進展なしの後も通常更新で再起動を繰り返しません。
- 直近 30 日でデータのある Codex の各日を、既に索引済みの時間別バケットへ展開できます。クリック時の JSONL 読み取りはゼロで、Claude / Codex は設定タイムゾーンと `HH:00` 表記を共有します。
- 週間 Codex クォータ観測は既存の索引処理中に圧縮され、上限付きでアカウント中立のローカル履歴として保持されます。そのため不規則なリセット境界も残せます。ポーリング、認証情報の読み取り、二重のスキャナーは追加しません。初回シード後の変更のない更新は、使用量本文を 0 バイトしか読みません。ローカルログに現れないリセットは推測できません。
- 直近 7 / 30 日の表示は、古いまたは膨張した期間投影を拒否し、タイムゾーン投影の再構築が終わるまで検証済みの日次集計へ一時的にフォールバックします。

パッケージのバージョンは変更していません。これはローカル検証候補であり、公開リリースではありません。
...
- Claude と Codex の全期間 / 比較ビューは、ローカル Token ログから過去の使用済み等価値を直接計算します。最新の有効な公式リセット観測を基準に、一意で重複しない週間期間の列を作り、各使用イベントは一つの期間にだけ数えます。利用できる観測がなければ、使用分のみの履歴を UTC の月曜から月曜の暦週にフォールバックします。重複して整合しない将来リセットは、別の quota series である場合に競合として扱います。同じ series の観測は一つの series のまま、低信頼度の近似として表示されることがあります。二つ目の「現在の期間」は作りません。期間範囲とリセット時刻は分けて表示します。Codex の account-wide `codex` 観測は過去と現在の期間の両方を装飾できます。リセット時刻が七日グリッドからずれる場合は、観測時刻を含む表示期間へ割り当てます。ファイルソース、リセットのずれ、日次スライスの境界越えは推定を可能にする一方で信頼度を下げ、近似として表示します。ファイルキーはアカウント ID ではないため、同じ home の対象ファイルをまとめて扱います。別の quota series、または観測のない期間は使用済み等価値だけを表示し、アカウント分割を作りません。現在の未使用分は表示せず、過去の未使用分は総上限推定がある場合だけ表示します。Codex の使用量は日単位のスライスで保存され、Token は一つの期間にだけ数えます。この表示規則はインデックス schema を変更せず、再構築も発生させません。履歴には現在の公式 API 単価を一貫して適用します。これは請求額や公式のサブスクリプション価格ではなく代理推定です。このパネルは既定で有効で、設定の `showWeeklyEquivalentValue` から非表示にできます。Claude はこのリリースからプロファイル別に上限観測を記録します。
