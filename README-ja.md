...
すべて同じインストール済み v2.3.1 VSIX と合成データから取得し、アカウント、パス、プロジェクト、スレッド、ログ本文を含みません。
...
- 直近 30 日は設定タイムゾーンの今日と過去 29 日です。合計は照合可能で、再更新は安定し、月は古い順に表示します。
- 構造化 reasoning effort をモデル名から推測しません。非ゼロの `unknown` は理由を示し、ゼロは表示しません。
- 上限付き匿名クォータ観測が不規則・同日リセットを保持します。有効な同一ウィンドウの割合から週間総枠を推定し、帰属が曖昧なら使用済みのみを表示します。
- 比較タブは既存の日次集計から Claude + Codex 統合活動ヒートマップを表示し、単一プロバイダーでも機能します。
- 決定的ローカル SVG、共有カード、Markdown、タイトル/範囲、プライバシープレビューに対応。公開 GitHub 送信は正確な宛先を別途確認します。
...
構造化証拠にないリセットは復元できません。複数ログインの帰属が曖昧なら使用済みのみです。
