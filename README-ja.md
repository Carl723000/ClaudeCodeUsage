...
- 設定は 1 つの通貨ドロップダウンに整理され、既定値は USD です。ほかの 13 通貨は
  2026-09-09 の ECB 参照レートを固定スナップショットとして使用します。レートは
  編集もオンライン取得もされず、基礎となる見積もりは USD のままです。
- Claude と Codex の全期間では月 → 日、直近 30 日では既存集計の日 → 時間を
  展開できます。マウス、キーボード、フォーカス、入れ子の折りたたみ、再読み込み後の
  復元が一致し、クリック時に JSONL を読みません。
- 通常のライブ更新では、タブ、展開階層、選択中の指標／時間、Optimizer の下書き、
  キーボードフォーカス、近くのスクロール位置を保持します。
- 各グラフには provider を含むローカライズ済みアクセシブル名があります。空の時間も
  軸、表、ツールチップ、選択可能な 0 値は保持しつつ、各棒の上の 0 表示は省略します。
  実際の利用があり価格不明の場合は引き続き `—` を表示します。
- provider watcher は上限付きバックオフで復旧し、変更のない Codex スレッドタイトル
  索引は検証済みメモリキャッシュを再利用してバックグラウンド処理を減らします。

レビュー済み Release ドラフトは `v2.3.2` です。手動公開ワークフローが、その tag からパッケージ版を設定します。

## v2.3.1 の新機能

- 正確な料金とコンテキスト情報が **GPT-6 Astra**（`gpt-6-astra`、
  105 万トークン）および **Claude Fable 5.1 / Mythos 5.1**
  （`claude-fable-5-1` / `claude-mythos-5-1`、100 万トークン）に対応しました。
  Fable 5.1 は専用のキャッシュ読み取り料金 `$0.25 / 100 万トークン` を使い、
  従来の Fable 5 履歴は変更しません。GPT-6 の API 相当額には公式 Standard
  の短コンテキスト料金を使用します。ローカル集計ログでは各リクエストが
  272K 入力を超えたか判定できないため、その場合のリクエスト全体への追加料金は
  含めません。
- Claude の見積もりでは、Anthropic 直接料金または AWS Bedrock の in-region
  オンデマンド料金を選択でき、現行の Opus・Sonnet・Haiku 系列を対象にします。
  切り替えると未変更のログも直ちに再計算され、Sonnet 5 には 2026 年 8 月 31 日の
  開始記念価格終了後の標準料金を使用します。
- 直近 30 日は設定タイムゾーンの今日と過去 29 日です。合計は照合可能で、再更新は安定し、月は古い順に表示します。Claude の Workflows サマリーも同じローリング範囲を使い、設定タイムゾーンでの開始日に基づいて各実行を集計します。チャート指標の切り替えやドリルダウンラベルの表示でも、日／月の使用量キーがホストのタイムゾーンによって前日・前月へずれることはありません。AI アドバイスと Usage Optimizer の保留期限も、設定タイムゾーンと UI 言語で表示します。
- 構造化 reasoning effort をモデル名から推測しません。非ゼロの `unknown` は理由を示し、ゼロは表示しません。
- 上限付き匿名クォータ観測が不規則・同日リセットを保持します。有効な同一ウィンドウの割合から、現在期間を含む総枠と未使用分の耐久性推定を表示します。近似・未帰属は低信頼度です。ローカルの Codex クォータ系列が重複していても、現在期間は最新の実測値による低信頼度の混合推定を表示し、帰属が曖昧な完了期間だけを使用済み表示に留めます。
- 比較タブは既存の日次集計から Claude + Codex 統合活動ヒートマップを表示し、単一プロバイダーでも機能します。
- プレビュー優先の共有スタジオは、決定的ローカル SVG、共有カード、Markdown、タイトル/範囲、プライバシープレビュー、調整可能な色に対応します。Academic Violet が既定で、強度マッピングは分位数（既定）、対数、線形から選べます。公開 GitHub 送信は正確な宛先を別途確認します。
- Codex の Token 構成は、重複しない「非キャッシュ入力 / キャッシュ入力 / 出力」スタックの上に **非キャッシュ使用量**（非キャッシュ入力 + 出力）を明示します。推論は引き続き出力の部分集合です。
- Codex はインデックス作成中も滑らかにスクロールします。スクロール状態は操作後に一度だけ保存され、ライブ進捗はページ全体を再構築せずテキストだけを更新します。モデル別の緑色サマリーは Claude と同じ意味になり、完全一致モデルの API 等価コストと価格カバレッジを表示します。未知モデルは `—`、effort は中立色の非キャッシュ Token 表示です。
- OpenAI 互換の Optimizer リクエストは `reasoning_effort` のみを送り、未対応のトップレベル `thinking` を送信しないため、[#94](https://github.com/ClaudeCodeUsage/ClaudeCodeUsage/issues/94) を解決します。
- Claude/Codex は密度、階層、フォーカス、ARIA、狭幅、明暗テーマの token を共有しつつ、指標の意味は分離します。
