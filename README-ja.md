...
- **処理済み** = 入力 + 出力、**新規入力を含む fresh** = 非キャッシュ入力 + 出力、**キャッシュ入力**は入力の部分集合、reasoning は出力の部分集合です。Codex のドルコストは推定せず、Claude / Codex / Compare は各プロバイダーの集計方法を分けて扱います。
- 既存ダッシュボードと同じ視覚言語で、主画面を 3 つに整理しました。**概要**は最近のタスク / 過去 7 日 / 過去 30 日 / すべての範囲でサマリー、推移、構成、最終観測上限、最近のタスクを表示します。**探索**はプロジェクト、セッション、モデルと推論強度を検索・絞り込み・並べ替え・ドリルダウンでき、親子関係も確認できます。**最適化の提案**は選択した範囲に従います。
...
- 各提案は、選択範囲に根拠がある場合だけ、観測、読みやすい根拠、構造的プロキシの説明、条件付きアクションを示します。根拠がなければ一般論を生成しません。
...
- Settings は前の主画面へ戻るプロバイダー対応の補助ページで、4 番目の主タブではありません。Codex の収集とローカル Codex 提案は別々に無効化できます。バックグラウンド監視遅延は既定 30 秒で、Off や長い間隔も選べます。
