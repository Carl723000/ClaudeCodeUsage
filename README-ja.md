...
- ローカルの Codex `sessions/**/*.jsonl` と `archived_sessions/**/*.jsonl` だけを読みます。認証情報と DB ファイルは対象外で、決定論的な分析はプロンプト、応答、コマンド、ツール引数本文を検査・利用・保持しません。
- **処理済み** = 入力 + 出力、**新規入力を含む fresh** = 非キャッシュ入力 + 出力、**キャッシュ入力**は入力の部分集合、reasoning は出力の部分集合です。Codex コストは推定しません。
- Claude / Codex / Compare 表示には、モデル、effort、root / child タスク比率、approval-reviewer 活動、インデックス網羅率、品質フラグ、**最終観測**された主要上限スナップショットがあります。
- ローカル提案は high effort、マルチエージェント、反復チェック、低キャッシュを示し、そのまま貼れる制約を提示します。バックグラウンド索引の監視遅延は既定 30 秒で、Off や長い間隔も選べます。
