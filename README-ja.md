...
- 上限付き匿名クォータ観測が不規則・同日リセットを保持します。有効な同一ウィンドウの割合から、現在期間を含む総枠と未使用分の耐久性推定を表示します。近似・未帰属は低信頼度とし、実際に重複するクォータ系列だけを使用済み表示に留めます。
- 比較タブは既存の日次集計から Claude + Codex 統合活動ヒートマップを表示し、単一プロバイダーでも機能します。
- プレビュー優先の共有スタジオは、決定的ローカル SVG、共有カード、Markdown、タイトル/範囲、プライバシープレビュー、調整可能な色に対応します。Academic Violet が既定で、分位強度により少数の極端日が通常日の差を消しません。公開 GitHub 送信は正確な宛先を別途確認します。
...
- Claude と Codex の全期間 / 比較ビューは、ローカル Token ログから過去の使用済み等価値を直接計算します。最新の有効な公式リセット観測を基準に、一意で重複しない週間期間の列を作り、各使用イベントは一つの期間にだけ数えます。利用できる観測がなければ、使用分のみの履歴を UTC の月曜から月曜の暦週にフォールバックします。同じ quota series の観測は一つの series のまま、低信頼度の近似として表示でき、二つ目の「現在の期間」は作りません。Codex の account-wide `codex` 観測は過去と現在の期間の両方を装飾でき、リセット時刻のずれ、ファイルソース、日次境界越えは信頼度を下げます。ファイルキーはアカウント ID ではないため、同じ home の対象ファイルをまとめて扱います。実際に重複する quota series、または観測のない期間は使用済み等価値だけを表示し、アカウント分割を作りません。一貫した観測期間は現在期間を含めて総枠と未使用分の耐久性推定を表示でき、帰属や境界が不確かな場合は値を隠さず信頼度を下げます。この表示規則はインデックス schema を変更せず、再構築も発生させません。履歴には現在の公式 API 単価を一貫して適用します。これは請求額、公式残高、公式のサブスクリプション価格ではなく代理推定です。このパネルは既定で有効で、設定の `showWeeklyEquivalentValue` から非表示にできます。Claude はこのリリースからプロファイル別に上限観測を記録します。
